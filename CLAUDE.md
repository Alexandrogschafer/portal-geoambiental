...
| Nascentes | ponto | azul claro `#5fb0e0` | `nascentes.geojson` | Sem página própria — sub-camada dentro de `mapas/hidrografia.html`, com toggle próprio no painel de Camadas (`initIndividualMapPage()` em `js/layers.js`) |
...
| Sistema Viário | linha | vermelho `#e63946` (mesmo tom do Desmatamento) | `sistema_viario.geojson` | Fonte: OpenStreetMap contributors, extraído via `osmnx` (`scripts/extract_osm_viario.py`) — licença ODbL, exige atribuição visível ("© OpenStreetMap contributors") sempre que a camada estiver ativa (ver `data/processed/FONTES.md`). Página própria: `mapas/sistema-viario.html`. Desligada por padrão no mapa geral (2410 segmentos poluiriam a visualização) |
...
Camadas novas devem seguir o mesmo padrão: entrada na tabela acima, checkbox no painel de camadas, `layerGroup` próprio em `js/layers.js`. Toda página individual de mapa (`mapas/*.html` com `<div id="map">`) desenha o contorno municipal (`limite_municipal.geojson`, branco tracejado) automaticamente via `initIndividualMapPage()` em `js/layers.js` — não precisa (e não deve) ser declarado como camada opcional por página. Para uma página com mais de uma camada de dado toggleável (ex.: Hidrografia + Nascentes), use `data-layer-keys="chave1,chave2"` no `<body>` em vez de `data-layer-key`; sem nenhum dos dois atributos, a página mostra só o contorno municipal (ex.: Virada Ambiental, APM). Se a camada ainda não tem dado processado, **não** deixe a ausência falhar silenciosamente (esse é o padrão do `fetchGeoJSON()` de `js/layers.js`, que só loga um aviso no console) — mostre um aviso visível na página, como em `mapas/virada-ambiental.html`/`mapas/apm.html`. Por padrão de contraste, todo polígono/linha usa contorno branco grosso (`HALO_WEIGHT`) em volta do preenchimento — sem isso, cores na mesma família do fundo (verde/marrom) ficam quase invisíveis sobre a imagem de satélite. `mapas/licenciamento.html` usa `js/licenciamento.js` (fora de `LAYER_CONFIG`) e por isso desenha o próprio contorno municipal separadamente, sem passar por `initIndividualMapPage()`.
